...
import { Loader2, Info, Maximize2, ZoomIn, ZoomOut, RotateCcw, Search, X, ArrowRight, CornerDownRight } from 'lucide-react';
...
const Topology: React.FC = () => {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (!originId) {
        showStatus(`Origin not found: "${originRaw}"`, 'error');
        return;
      }
      ...
      if (!destQuery) {
        ...
        if (!destId) {
          showStatus(`Destination not found: "${destRaw}"`, 'error');
          return;
        }
  ...
  useEffect(() => {
    getTopologyData().then(res => {
      ...
      nodes.forEach((n: NodeData) => {
        ...
        const incoming = inDeg[n.id] || 0;
        const size = role === 'tier1' ? 40 : Math.round(12 + (incoming / maxIn) * 22);
        const shortName = n.name && n.name.length > 22 ? n.name.substring(0, 22) + '…' : (n.name || '');

        elements.push({
          data: {
            id: cyId,
            label: `AS${n.asn_number || n.id}\n${shortName}`,
            role,
      ...
      if (containerRef.current) {
          ...
          style: [
            ...
            },
            {
              ...
              style: {
                ...
                'text-outline-color': '#fff', 'text-outline-width': 2, 'z-index': 9999, 'opacity': 1
              }
            },
            {
            ...
            },
            {
              ...
              style: {
                ...
                'color': '#1e293b', 'font-size': 11, 'min-zoomed-font-size': 0,
                'text-outline-color': '#fff', 'text-outline-width': 2, 'z-index': 10000, 'opacity': 1
              }
            },
            {
        ...
        cyRef.current.on('tap', 'node', (evt) => {
          handleNodeClick(evt.target.data('asn_number'));
        });
  ...
  return (
    <div className="relative h-[calc(100vh-140px)] -m-4 md:-m-6 bg-slate-50 flex flex-col overflow-hidden border border-slate-200">
      {/* Search status toast */}
      {searchStatus && (
        <div className={`fixed top-24 left-1/2 -translate-x-1/2 z-100 px-6 py-3 rounded-2xl shadow-xl font-bold text-sm tracking-tight border flex items-center gap-3 animate-in fade-in slide-in-from-top-4 ${
          searchStatus.type === 'error' ? 'bg-red-50 border-red-200 text-red-700' :
          searchStatus.type === 'success' ? 'bg-emerald-50 border-emerald-200 text-emerald-800' :
          'bg-sky-50 border-sky-200 text-sky-800'
        }`}>
          {searchStatus.type === 'error' ? '❌' : searchStatus.type === 'success' ? '✅' : 'ℹ️'}
      ...
      <div className="bg-white border-b border-slate-200 p-4 relative z-20 shadow-sm flex flex-col md:flex-row items-center gap-4">
        <div className="flex-1 flex items-center gap-2 w-full">
            <div className="relative flex-1">
                <input 
                    type="text" 
                    placeholder="Origin (ASN / IP / Domain)"
                    className="w-full pl-4 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-xl focus:border-sky-500 focus:ring-0 transition-all font-mono text-xs text-slate-700 outline-none"
                    value={originInput}
        ...
        <div className="flex gap-2 w-full md:w-auto">
            <button 
                ...
                className="flex-1 md:flex-none px-6 py-2 bg-slate-900 text-white font-bold rounded-xl hover:bg-slate-800 transition-all text-xs"
            >
                Find Path
            </button>
            <button 
                onClick={() => { setOriginInput(''); setDestInput(''); resetHighlights(); }}
                className="px-4 py-2 border border-slate-200 rounded-xl hover:bg-slate-50 transition-colors text-xs font-bold text-slate-500"
            >
                Clear
            </button>
            <button 
              onClick={() => setIsLegendOpen(!isLegendOpen)}
              className="md:hidden px-4 py-2 border border-slate-200 rounded-xl text-xs font-bold text-slate-600"
            >
              Legend
            </button>
        </div>
        ...
        {pathInfo && (
            <div className="absolute top-6 left-6 z-10 bg-white/90 backdrop-blur-md border border-slate-200 p-6 rounded-4xl shadow-2xl w-full max-w-[340px] max-h-[70%] flex flex-col animate-in fade-in slide-in-from-left-4">
                <div className="flex items-center justify-between mb-4">
                    <h3 className="flex items-center gap-2 text-xs font-black text-sky-600 uppercase tracking-widest">
                        <Search size={14} /> {pathInfo.title}
                    </h3>
                        ...
                        <div key={pIdx} className="mb-6 last:mb-0 pb-4 border-b border-slate-100 last:border-0">
                            <div className="flex items-center gap-2 mb-3">
                                ...
                                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
                                    Path {pIdx + 1} ({path.length - 1} hops)
                                </span>
                            </div>
                            <ul className="space-y-3">
                                {path.map((nodeId, nIdx) => {
                                    ...
                                    const isOrigin = nIdx === 0;
                                    const isDest = nIdx === path.length - 1;
                                    return (
                                        <li key={nIdx} className="flex gap-3 text-xs">
                                            {nIdx > 0 && <span className="text-slate-300 font-bold"><CornerDownRight size={12} /></span>}
                                            <div className="flex flex-col">
                                                <span className={`font-bold ${isOrigin ? 'text-emerald-600' : isDest ? 'text-violet-600' : 'text-slate-700'}`}>
                                                    {label}
                                                </span>
        ...
        <div className={`absolute bottom-6 right-6 bg-white/80 backdrop-blur-md border border-slate-200 p-6 rounded-4xl shadow-xl w-64 space-y-4 transition-all z-10 ${isLegendOpen ? 'opacity-100 scale-100' : 'opacity-0 scale-95 pointer-events-none'}`}>
            <div className="flex items-center justify-between">
                <h3 className="flex items-center gap-2 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                    <Info size={14} className="text-sky-500" /> Infrastructure Node Types
                </h3>
                <button onClick={() => setIsLegendOpen(false)} className="md:hidden text-slate-400"><X size={14}/></button>
            </div>
            <div className="space-y-3">
                <div className="flex items-center gap-3">
                    <div className="w-3 h-3 rounded-full bg-red-500 border-2 border-red-800 shadow-sm" />
                    <span className="text-xs font-bold text-slate-700">Tier-1 / Backbone</span>
                </div>
                <div className="flex items-center gap-3">
                    <div className="w-3 h-3 rounded-full bg-blue-500 border-2 border-blue-800 shadow-sm" />
                    <span className="text-xs font-bold text-slate-700">Transit / Provider</span>
                </div>
                <div className="flex items-center gap-3">
                    <div className="w-3 h-3 rounded-full bg-emerald-500 border-2 border-emerald-800 shadow-sm" />
                    <span className="text-xs font-bold text-slate-700">Access / Origin</span>
                </div>
                <div className="flex items-center gap-3">
                    <div className="w-3 h-3 rounded-full bg-amber-500 border-2 border-amber-800 shadow-sm" />
                    <span className="text-xs font-bold text-slate-700">Standalone Node</span>
                </div>
            </div>
            <div className="pt-4 border-t border-slate-100">
                <p className="text-[10px] leading-relaxed text-slate-400 font-medium italic">
                    Node size indicates relative peering density (in-degree).
                </p>
            </div>
        </div>
              ...
              <div className="bg-white w-full max-w-[340px] rounded-4xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200" onClick={(e) => e.stopPropagation()}>
                  <div className="p-6 border-b border-slate-100 flex items-start justify-between">
                      <div>
                          ...
                          <p className="text-xs font-medium text-slate-500 line-clamp-1">{popupData.name}</p>
                      </div>
                  ...
                  <div className="p-6 space-y-3">
                      <div className="flex justify-between items-center py-2 border-b border-slate-50">
                          <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Network Type</span>
                          <span className="text-xs font-bold text-sky-600 bg-sky-50 px-2 py-0.5 rounded">{popupData.network_type}</span>
                      </div>
                      <div className="flex justify-between items-center py-2 border-b border-slate-50">
                          <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Status</span>
                          <span className={`text-xs font-bold px-2 py-0.5 rounded ${popupData.network_status?.toLowerCase().includes('active') ? 'bg-emerald-50 text-emerald-600' : 'bg-slate-100 text-slate-600'}`}>
                              {popupData.network_status}
                          </span>
                      </div>
                      <div className="flex justify-between items-center py-2 border-b border-slate-50">
                      ...
                      <div className="flex justify-between items-center py-2">
                          <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">IPv4 Prefixes</span>
                          <span className="text-xs font-bold text-slate-900">{popupData.prefixes_v4}</span>
                      </div>
                  </div>

                  <div className="p-6 pt-0">
                      <Link 
                        to={`/asn/${popupData.asn_number}`} 
                        className="w-full py-4 bg-slate-900 text-white rounded-2xl font-bold flex items-center justify-center gap-2 hover:bg-slate-800 transition-all text-sm"
                      >
                        Deep Intelligence <ArrowRight size={18} />
                      </Link>
                  </div>
              </div>
          </div>
